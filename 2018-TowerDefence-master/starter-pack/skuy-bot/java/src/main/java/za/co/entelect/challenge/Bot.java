...
public class Bot {
    ...
    public String run() {
        ...
        for (int i = 0; i <= 7; i++) {
            ...
            if (getEnergy(PlayerType.A) >= 20 && checkNumberEnergyBuilding(i)) {
                int row = 0;
	            while (row<=1){
	                if (isCellEmpty(row, i) == true){
	                    return command = buildCommand(row, i, BuildingType.ENERGY);
	                }
	                else{
	                    row++;
	                }
        ...
        if (fullEnergy == 1 && getEnergy(PlayerType.A)>=30) {
            ...
            int max = -9;
            for (int i = 0; i < 8; i++){
                enemyBuildingPrioOnRow = getAllBuildingsForPlayer(PlayerType.B, b -> b.buildingType == BuildingType.ENERGY, i).size();
                enemyBuildingPrioOnRow -= getAllBuildingsForPlayer(PlayerType.B, b -> b.buildingType == BuildingType.DEFENSE, i).size();
                enemyBuildingPrioOnRow -= getAllBuildingsForPlayer(PlayerType.B, b -> b.buildingType == BuildingType.ATTACK, i).size();
                int myAttackOnRow = getAllBuildingsForPlayer(PlayerType.A, b -> b.buildingType == BuildingType.ATTACK, i).size();
                if(getAllBuildingsForPlayer(PlayerType.B, b -> b.buildingType == BuildingType.ENERGY, i).size()==0 && getAllBuildingsForPlayer(PlayerType.B, b -> b.buildingType == BuildingType.DEFENSE, i).size()==0 && getAllBuildingsForPlayer(PlayerType.B, b -> b.buildingType == BuildingType.ATTACK, i).size()==0 ){
                	int row = 5;
		            while (row>0){
		                if (isCellEmpty(row, baris) == true){
		                    return command = buildCommand(row, baris, BuildingType.ATTACK);
		                }
		                else{
		                    row--;
		                }
                ...
                if (enemyBuildingPrioOnRow > max && myAttackOnRow < 5){
                    max = enemyBuildingPrioOnRow;
                    baris = i;
                }
            }
            int x = 7;
    ...
    */
    private boolean checkNumberDefenseBuilding(int row){
        ...
        if(enemyAttackOnRow==0 && row!=6 && row!=7 && myDefenseOnRow==0){
            return false;
        }
        else if(enemyAttackOnRow<=3 && myDefenseOnRow<1 && enemyAttackOnRow>=1){
            return true;
        }
        else if(enemyAttackOnRow>=4 && myDefenseOnRow<2){
        	return true;
        }
        return false;
    }
    private boolean checkNumberEnergyBuilding(int row){
        int myDefenseOnRow = getAllBuildingsForPlayer(PlayerType.A, b -> b.buildingType == BuildingType.ENERGY, row).size();
        int enemyAttackOnRow = getAllBuildingsForPlayer(PlayerType.B, b -> b.buildingType == BuildingType.ATTACK, row).size();
        if(enemyAttackOnRow==0 && row!=7 && row!=6 && myDefenseOnRow==0){
            return true;
        }
        else if(enemyAttackOnRow<=3 && myDefenseOnRow<1 && enemyAttackOnRow>=1){
            return true;
